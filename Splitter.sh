...
#############################################################################
# This script produces a split membrane out of a membrane with CHARMM36 FF. #       
...
echo -n "Enter the name of your gro file (example: step7_10):"
read GRO
GRO="$GRO"
...
cp $GRO.gro charmm.gro
cp $GRO.tpr charmm.tpr

residues=`tail -n +3 charmm.gro | head -n-1 | awk -v FIELDWIDTHS="5 4 99" '{print $2}' | uniq` 
...
echo "name 1 MEMB"
echo q
    } | gmx make_ndx -f charmm.gro -o index.ndx

# Remove the title and box size

awk -F '|' -v OFS='|' '{sub(/^.............../, "& "); print}' charmm.gro > system.gro
...
tail -n 1 charmm.gro > footer.txt
...
gmx editconf -f charmm.gro -o tmp_charmm.gro -box $X $Y $Z -c

echo SYSTEM | gmx traj -f tmp_charmm.gro -s charmm.tpr -n index.ndx -ox system.xvg -com
